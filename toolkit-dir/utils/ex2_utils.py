...
def custom_function():
    ...
    responses[800, 800] = 100
    responses[800, 500] = 55
    responses[500, 800] = 65
    responses[500, 500] = 80
    return gausssmooth(responses, 110)

def get_kernels(height, width):
    ...
    limitX = w2//2
    limitY = h2//2
    rowX = [i for i in range(-limitX, limitX+1)]
    kernelX = np.array([rowX for i in range(h2)])
    kernelY = np.array([np.full(w2, i) for i in range(-limitY, limitY+1)])
...
def extract_histogram(patch, nbins, weights=None):
    ...
    #print(len(bin_idxs.flatten()))
    #print(len(weights.flatten()))
